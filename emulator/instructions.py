...
def load(memory_banks: list[emulator.memory.Memory], instruction_address: int):
    address_to_load = get_arg_address(memory_banks[2], instruction_address, 0)
    memory_banks[0].set_value(address_to_load, memory_banks[2].get_value(instruction_address + I_SIZE + A_SIZE, A_SIZE))

def bsr(memory_banks: list[emulator.memory.Memory], instruction_address: int):
    address_to_shift = get_arg_address(memory_banks[2], instruction_address, 0)
    bits = memory_banks[0].get_value(address_to_shift, A_SIZE)
    bits.pop()
    bits.insert(0, 0)
    memory_banks[0].set_value(address_to_shift, bits)

def add(memory_banks: list[emulator.memory.Memory], instruction_address: int):
    address_to_add_to = get_arg_address(memory_banks[2], instruction_address, 0)
    address_to_add_from = get_arg_address(memory_banks[2], instruction_address, 1)

    arr1 = memory_banks[0].get_value(address_to_add_to, A_SIZE)
    arr2 = memory_banks[0].get_value(address_to_add_from, A_SIZE)
    ...
    memory_banks[0].set_value(FC[0], [carry])

    memory_banks[0].set_value(address_to_add_to, result)
