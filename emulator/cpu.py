...
class CPU:
    ...
    def tick(self):
        ...
        instruction_address = emulator.memory.bit_array_to_int(self.memory_banks[MEMBANK.REG.value].get_value(RI[0], RI[1]))
        instruction_bits = self.memory_banks[MEMBANK.ROM.value].get_value(instruction_address, I_SIZE)
        instruction = emulator.memory.bit_array_to_int(instruction_bits)

        if instruction == 1: # hits return
            print("Hit ret")
            stack_size = emulator.memory.bit_array_to_int(self.memory_banks[MEMBANK.STK.value].get_value(0, A_SIZE))
            if stack_size == 0:
                self.running = False
            else:
                self.memory_banks[MEMBANK.REG.value].set_value(PC[0], self.memory_banks[MEMBANK.STK.value].get_value(A_SIZE * stack_size, A_SIZE))
                self.memory_banks[MEMBANK.STK.value].set_value(0, emulator.memory.int_to_bit_array(stack_size - 1, A_SIZE))
            return

        instruction_size = I_SIZE + A_SIZE * compiler.instructions.INSTRUCTION_SIZE[instruction]
        
        self.memory_banks[MEMBANK.REG.value].add_value(PC[0], instruction_size, PC[1])
